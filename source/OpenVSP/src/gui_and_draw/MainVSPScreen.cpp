...
MainVSPScreen::MainVSPScreen( ScreenMgr* mgr ) : ActionScreen( mgr )
{
    ...
    m_FileNameBox->copy_label( vsp::l10n::Tr( "File Name:" ) );
...
void MainVSPScreen::BoldEntries( const string &mpath )
{
    ...
    const string translated_path = vsp::l10n::TrMenuPath( mpath );
...
void MainVSPScreen::SetFileLabel( const string &fname )
{
    ...
    string label = vsp::l10n::Tr( "File Name:" );
    label.append( " " );
    label.append( fname );
...
{
    const string prompt = vsp::l10n::Tr( string( "New will lose any changes.  Do you want to proceed (discarding changes) or save?" ) );
    const string cancel = vsp::l10n::Tr( string( "Cancel" ) );
    const string discard = vsp::l10n::Tr( string( "Discard" ) );
    const string save = vsp::l10n::Tr( string( "Save" ) );
    switch( fl_choice( prompt.c_str(), cancel.c_str(), discard.c_str(), save.c_str() ) )
    {
        case(0):  // Cancel
            return;
...
{
    const string prompt = vsp::l10n::Tr( string( "VSP is exiting. Save or discard your changes." ) );
    const string cancel = vsp::l10n::Tr( string( "Cancel" ) );
    const string discard = vsp::l10n::Tr( string( "Discard" ) );
    const string save = vsp::l10n::Tr( string( "Save" ) );
    switch( fl_choice( prompt.c_str(), cancel.c_str(), discard.c_str(), save.c_str() ) )
