...
void VSPAEROPlotScreen::ConstructCpSliceCaseString( char *strbuf, size_t strsiz, Results *res, int case_num )
{
    if ( strbuf && res )
    {
        ...
        snprintf( strbuf, strsiz, "%s %d: %s=%.2g, %s=%.2g, %s=%.2g", vsp::l10n::Tr( "Case" ), case_num, vsp::l10n::Tr( "Alpha" ), alpha, vsp::l10n::Tr( "Beta" ), beta, vsp::l10n::Tr( "Mach" ), mach );
    }
}
...
void VSPAEROPlotScreen::UpdateCpSliceCutBrowser()
{
    ...
    for ( unsigned int iCut = 0; iCut < m_NumCpCuts; iCut++ )
    {
        ...
        if ( res )
        {
            ...
            char strbuf[1024];
            snprintf( strbuf, sizeof( strbuf ),  "%s %d: %c= %4.2f", vsp::l10n::Tr( "切面" ), cut_num, type_char, location );
...
void VSPAEROPlotScreen::RedrawSweepPlot()
{
    ...
    if ( ( nPolarPoints > 0 ) && ( !xDataSetNames.empty() ) && ( !yDataSetNames.empty() ) )
    {
        ...
        {
            m_SweepPlotCanvas->current_x()->copy_label( vsp::l10n::Tr( "[multiple]" ) );
        }
        ...
        {
            m_SweepPlotCanvas->current_y()->copy_label( vsp::l10n::Tr( "[multiple]" ) );
        }
...
void VSPAEROPlotScreen::RedrawCpSlicePlot()
{
    ...
    if ( ( !m_CpSliceCaseSelectedResultIDs.empty() ) && ( !m_CpSliceCutSelectedResultIDs.empty() ) )
    {
        for ( size_t iCut = 0; iCut < m_CpSliceCutSelectedResultIDs.size(); iCut++ )
        {
            for ( int iCase = 0; iCase < m_CpSliceCaseSelectedResultIDs.size(); iCase++ )
            {
                if ( m_CpSliceCutSelectedResultIDs[iCut] == m_CpSliceCaseSelectedResultIDs[iCase] )
                {
                    ...
                    char strbuf[1024];
                    snprintf( strbuf, sizeof( strbuf ),  "%s %d, %s %d", vsp::l10n::Tr( "Cut" ), cut_num, vsp::l10n::Tr( "Case" ), case_num );
        ...
        {
            char strbuf[1024];
            snprintf( strbuf, sizeof( strbuf ),  "%s：%c", vsp::l10n::Tr( "位置" ), ( 88 + pos_type_vec[0] ) ); // ASCII X: 88; Y: 89; Z: 90

            m_CpSlicePlotCanvas->current_x()->copy_label( strbuf );
        }
        else
        {
            m_CpSlicePlotCanvas->current_x()->copy_label( vsp::l10n::Tr( "Position [multiple]" ) );
        }
...
void VSPAEROPlotScreen::PlotConvergence( const string &resultID, vector <string> yDataSetNames, bool expandOnly, int icase )
{
    ...
    else if ( strcmp( res->GetName().c_str(), "VSPAERO_History" ) == 0 )
    {
        ...
        m_ConvergencePlotCanvas->current_x()->copy_label( vsp::l10n::Tr( "Wake Iteration #" ) );

    }
    else
    {
        m_ConvergencePlotCanvas->copy_label( vsp::l10n::Tr( "PLOT ERROR - INVALID RESULT TYPE" ) );
    }
...
void VSPAEROPlotScreen::PlotLoadDistribution( const string &resultID, vector <string> yDataSetNames, bool expandOnly, int icase )
{
    ...
    {
        m_LoadDistPlotCanvas->copy_label( vsp::l10n::Tr( "PLOT ERROR - INVALID RESULT TYPE" ) );
    }
}
...
void VSPAEROPlotScreen::PlotUnsteady( const string &resultID, vector <string> yDataSetNames, bool expandOnly, int icase )
{
    ...
    else if ( strcmp( res->GetName().c_str(), "VSPAERO_History" ) == 0 || strcmp( res->GetName().c_str(), "VSPAERO_Rotor" ) == 0 || strcmp( res->GetName().c_str(), "VSPAERO_Group" ) == 0 )
    {
        ...
        m_UnsteadyPlotCanvas->current_x()->copy_label( vsp::l10n::Tr( "Time (sec)" ) );
    }
    else
    {
        m_UnsteadyPlotCanvas->copy_label( vsp::l10n::Tr( "PLOT ERROR - INVALID RESULT TYPE" ) );
    }
