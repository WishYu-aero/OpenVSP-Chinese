...
ScreenMgr::ScreenMgr( Vehicle* vPtr )
{
    ...
    #ifdef _WIN32
        // Prefer common Windows CJK fonts for translated labels.
        Fl::set_font(FL_HELVETICA,     " Microsoft YaHei UI");
        Fl::set_font(FL_HELVETICA + 1, "BMicrosoft YaHei UI");
        Fl::set_font(FL_HELVETICA + 2, "IMicrosoft YaHei UI");
        Fl::set_font(FL_HELVETICA + 3, "PMicrosoft YaHei UI");
    #endif
...
void ScreenMgr::ShowScreen( int id )
{
    if ( id >= 0 && id < vsp::VSP_NUM_SCREENS )
    {
        if ( IsGUIScreenDisabled( id ) )
        {
            ...
            if ( bs )
            {
                Alert( vsp::l10n::Tr( bs->GetTitle() ) + vsp::l10n::Tr( " screen is disabled." ) );
            }
            else
            {
...
string ScreenMgr::FileChooser( const string &title, const string &filter, int mode, const string &preset_fname, const string &dir)
{
    ...
    if ( fc_type == vsp::FC_OPENVSP )
    {
        ...
        return sfc->FileChooser( vsp::l10n::Tr( title ), filter, mode, preset_fname, dir );
    }
    else // FC_NATIVE
    {
...
string ScreenMgr::NativeFileChooser( const string &title, const string &filter, int mode, const string &preset_fname, const string &dir )
{
    ...
    m_NativeFileChooser->title( vsp::l10n::Tr( title ).c_str() );
...
{
    std::string text = vsp::l10n::Tr( (char*)data );
