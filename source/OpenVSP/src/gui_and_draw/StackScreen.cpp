...
void StackScreen::GuiDeviceCallBack( GuiDevice* gui_device )
{
    ...
    {
        const string prompt = vsp::l10n::Tr( "Clear all XSECs and init to preset?" );
        const string cancel_text = vsp::l10n::Tr( "Cancel" );
        const string okay_text = vsp::l10n::Tr( "Okay" );
        switch( fl_choice( prompt.c_str(), cancel_text.c_str(), okay_text.c_str(), 0 ) )
        {
            case(0):
                break;
            case(1):
