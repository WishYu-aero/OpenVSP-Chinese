...
void BasicScreen::SetTitle( const string& title )
{
    ...
    {
        m_Title = title;
        m_FL_TitleBox->copy_label( vsp::l10n::Tr( m_Title ).c_str() );
    }
}
...
Fl_Group* TabScreen::MakeTab( const string& title )
{
    ...
    grp->copy_label( vsp::l10n::Tr( title ).c_str() );
...
bool GeomScreen::Update()
{
    ...
    std::vector< std::string > material_names = MaterialMgr.GetNames();
    for ( int i = 0; i < (int)material_names.size(); i++ )
    {
        if( mat->m_Name == material_names[i] )
        {
            m_MaterialChoice.SetVal( i + 1 );
            break;
        }
    }
...
void GeomScreen::UpdateMaterialNames()
{
    ...
    m_MaterialChoice.AddItem( vsp::l10n::Tr( "DEFAULT" ) );
    for( int i = 0; i < (int) matNames.size(); i++ )
    {
        m_MaterialChoice.AddItem( vsp::l10n::Tr( matNames[i] ) );
    }
}
...
void SkinScreen::GuiDeviceCallBack( GuiDevice* gui_device )
{
    ...
    {
        const string prompt = vsp::l10n::Tr( string( "Disable and clear all skinning values from stack?" ) );
        const string cancel = vsp::l10n::Tr( string( "Cancel" ) );
        const string clear_skinning = vsp::l10n::Tr( string( "Clear Skinning" ) );
        switch ( fl_choice( prompt.c_str(),
                            cancel.c_str(),
                            clear_skinning.c_str(),
                            0 ) )
        {
            case (0):
            {
