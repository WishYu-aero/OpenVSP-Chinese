    ...
    if ( device == &m_OKPreserveCode )
    {
        ...
        if ( edit_link )
        {
            ...
            if ( edit_link->DuplicateVarName( name ) )
            {
                ...
                const string prompt = vsp::l10n::Tr( "%s is not unique, use %s instead?" );
                const string cancel_text = vsp::l10n::Tr( "Cancel" );
                const string suggestion_text = vsp::l10n::Tr( "Use Suggestion" );
                if ( 0 == fl_choice( prompt.c_str(), cancel_text.c_str(), suggestion_text.c_str(),  0, name.c_str(), new_name.c_str() ) )
                {
                    cancel = true;
                }
                name = new_name;
