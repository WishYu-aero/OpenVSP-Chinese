...
void ParmLinkScreen::GuiDeviceCallBack( GuiDevice* device )
{
    ...
    else if( device == &m_AddLink )
    {
        ...
        if ( !success )
        {
            fl_alert( "%s", vsp::l10n::Tr( "Error: Identical Parms or Already Linked" ) );
        }
        m_SelectedLinks.clear();
    }
    ...
    else if ( device == &m_LinkConts )
    {
        ...
        if ( !success )
        {
            fl_alert( "%s", vsp::l10n::Tr( "Error: Identical Comps" ) );
        }
        m_SelectedLinks.clear();
    }
    else if ( device == &m_LinkGroups )
    {
        ...
        if ( !success )
        {
            fl_alert( "%s", vsp::l10n::Tr( "Error: Identical Group" ) );
        }
        m_SelectedLinks.clear();
    }
