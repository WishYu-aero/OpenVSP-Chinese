    ...
    if( veh )
    {
        ...
        else
        {
            ...
            for( size_t iMach = 0; iMach < VSPAEROMgr.m_MachNpts.Get(); iMach++ )
            {
                ...
                {
                    AddOutputText( m_SolverDisplay, vsp::l10n::Tr( "WARNING: Possible transonic Mach number detected - transonic flow is not supported.\n\n" ) );
                    break;
                }
            }
    ...
    if( veh )
    {
        ...
        else if( device == &m_ExportResultsToCsvButton )
        {
            ...
            if ( !fileName.empty() )
            {
                ...
                {
                    fl_alert( vsp::l10n::Tr( "File export failed\nFile: %s" ), fileName.c_str() );
                }
            }
        }
...
void VSPAEROScreen::UpdateCaseSetup()
{
    ...
    m_CullFracSlider.SetButtonName( vsp::l10n::Tr( "Cull Orphans" ) );
